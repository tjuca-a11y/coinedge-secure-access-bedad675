...
export function DashboardLayout({ children, title, subtitle, hideHeader }: DashboardLayoutProps) {
  ...
  return (
    <SidebarProvider>
      <div className="min-h-screen flex w-full safe-area-top">
        <AppSidebar />
        <div className="flex-1 flex flex-col min-w-0">
          {!hideHeader && (
            <header className="h-14 md:h-16 border-b border-border bg-card flex items-center justify-between px-3 md:px-6 sticky top-0 z-10 safe-area-left safe-area-right">
          ...
          <main className="flex-1 p-3 md:p-6 pb-24 md:pb-6 overflow-auto scroll-smooth-ios hide-scrollbar safe-area-left safe-area-right">
            <KycBanner />
            {children}
          </main>
        </div>
